...
public class newStudySubmodules extends WizardStep{
    ...
    final String LOG_TAG = "newStudySubmodules";
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //Si la API no esta funcionant al servidor no pot agafar les dades
        /*
            //Retrieve schools on JSON format
            String json_studysubmodules_list = settings.getString("studysubmodules_list", "");
            Log.d(LOG_TAG,"###### json_studysubmodules_list: " + json_studysubmodules_list);
    
            Gson gson = new Gson();
            mStudySubmodules = gson.fromJson(json_studysubmodules_list, StudySubmodules[].class);
        */
        ...
        StudySubmodules1.setName("patata1");
        StudySubmodules2.setName("patata2");
        StudySubmodules3.setName("patata3");

        StudySubmodules1.setShortname("patata1 nom complet");
        ...
        for (int i = 0; i < mStudySubmodules.length; i++) {
            Log.d("########## TEST: ", mStudySubmodules[i].getShortname());
            // Create a Card
            Card card_on_list = new Card(getActivity());

            // Create a CardHeader and add Header to card_on_list
            CardHeader header = new CardHeader(getActivity());
            header.setTitle(mStudySubmodules[i].getShortname());

            card_on_list.addCardHeader(header);

            card_on_list.setId(mStudySubmodules[i].getId());
            card_on_list.setTitle(mStudySubmodules[i].getName());
            ...
            if (mStudySubmodules[i].getModuleId()!=""){
                thumb.setUrlResource(mStudySubmodules[i].getModuleId());
            } else {
    ...
    private boolean check_all_data_is_ok(int position) {
        StudySubmodules selected_school = mStudySubmodules[position];
        Log.d(LOG_TAG, "Checking school at position " + position + " ( " + selected_school.getShortname() + " )" );

        String login_api_url = selected_school.getCourseid();
        Log.d(LOG_TAG, "getCourseid (courseid): " + login_api_url);             //Falta arreglar
        String api_url = selected_school.getOrder();
